...
.setAuthor({
   ...
   var LogChannel = client.guilds.cache.get(takachicim.GuildID).channels.cache.find((channel) => channel.id === settings.ÖneriİstekSikayetChannelID);
...
var LogChannel = client.guilds.cache.get(takachicim.GuildID).channels.cache.find((channel) => channel.id === settings.CanlıDestekLogChannelID);
...
`)
.setAuthor({ name: "Canlı Destek", iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({ dynamic: true, size: 2048 }) })
...
components : []
  ...
  client.guilds.cache.get(takachicim.GuildID).channels.create({ name: `${interaction.user.displayName}-destek`,
    ...
    permissionOverwrites: [{
      ...
      {
        id: client.guilds.cache.get(takachicim.GuildID).roles.everyone,
...
if(interaction.customId === "basvuru") {
  ...
  if(!client.guilds.cache.get(takachicim.GuildID).members.cache.get(interaction.user.id).roles.cache.has(conf.ekipRolu)) {
    return interaction.reply({ content: ":x: Tagın olmadığı için başvuramazsın.", ephemeral: true })
  } else if([conf.staffs].some(role2 => client.guilds.cache.get(takachicim.GuildID).members.cache.get(interaction.user.id).roles.cache.get(role2))) {
