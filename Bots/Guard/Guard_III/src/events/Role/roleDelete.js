...
const newRole = await role.guild.roles.create({
  ...
  let kanalPermVeri = data.channelOverwrites.filter(e => client.guilds.cache.get(takachicim.GuildID).channels.cache.get(e.id))
...
for (let index = 0; index < availableBots.length; index++) {
    ...
    return true;}
    await member.roles.add(newRole.id).then(e => {console.log(`Oto Silinen Rol kurulumundan sonra ${bot.user.tag} - ${member.user.displayName} adlı üye ${newRole.name} rolünü aldı.`);}).catch(e => {console.log(`[${newRole.id}] Olayından sonra ${bot.user.username} - ${member.user.displayName} adlı üyeye rol veremedim.`);});});
