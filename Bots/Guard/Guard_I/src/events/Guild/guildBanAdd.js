...
if (BanLimit[entry.executor.id] && BanLimit[entry.executor.id].Now + 1 > takachicim.Guard.Limit.Ban) {
    if (victimMember) {
...
.setDescription(`
${entry.executor} yetkilisi **${takachicim.Guard.Limit.Ban}** Ban limitini anasını çükt ıhm geçtiği için kendisi banlandı ve banlanan üyenin banı kaldırıldı.
─────────────────────
...
var filter = (button) => conf.sahipRolu.some(x => x == button.member.roles.cache.has(x)) || takachicim.owners.includes(button.user.id);
...
.setThumbnail(entry.executor.avatarURL({ dynamic: true }))
.setDescription(`
${entry.executor} yetkilisi kalan Ban Limit: **${1}/${takachicim.Guard.Limit.Ban}**.
─────────────────────
Yetkili: (${entry.executor} - \`${entry.executor.id}\`)
Kullanıcı: \`${member.user.tag}\` - \`${member.user.id}\`
─────────────────────
...
.setThumbnail(entry.executor.avatarURL({ dynamic: true }))
.setDescription(`
${entry.executor} yetkilisi kalan Ban Limit: **${BanLimit[entry.executor.id].Now}/${takachicim.Guard.Limit.Ban}**.
─────────────────────
Yetkili: (${entry.executor} - \`${entry.executor.id}\`)
Kullanıcı: \`${member.user.tag}\` - \`${member.user.id}\`
─────────────────────
