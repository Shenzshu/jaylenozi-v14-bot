...
module.exports = async (message) => {
    ...
    if (takachicim.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(message.content))) return sendChat(message, "Umarım küfür etmedin yoksa sikerim belanı güzel kardeşim küfür senin o güzel diline hiç yakışıyormu ayıp edep erkan öğrenin az")
    ...
    if ((takachicim.Guard.capsEngel) && (Caps / message.content.length) >= 0.7) return sendChat(message, "Caps-Lock kullanmayı lütfen azaltalım")

    if (takachicim.Guard.reklamEngel && message.content.match(inviteEngel)) {
    ...
    if(takachicim.Guard.reklamEngel && reklamlar.some(word => message.content.toLowerCase().includes(word))) return sendChat(message, "Lütfen reklam yapmayı bırakınız")
}
...
client.on("messageCreate", async (message) => {
  if(message.webhookID || message.author.bot || message.channel.type === ChannelType.DM) return;
  if (await client.checkPermission(client, message.author.id, "full") || await client.checkPermission(client, message.author.id, "chatguard")) return;
  if (takachicim.Guard.spamEngel == false) return;
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (takachicim.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(newMessage.content))) newMessage.delete().catch(err => {});
    ...
    if (takachicim.Guard.reklamEngel && newMessage.content.match(inviteEngel)) {
    ...
    if(takachicim.Guard.kufurEngel && reklamlar.some(word => newMessage.content.toLowerCase().includes(word))) return newMessage.delete().catch(err => {})
});
    ...
    async function sendChat(message, content) {
        if ((Number(getLimit.get(`${message.member.id}`))) == 3) {
            ...
            client.guilds.cache.get(takachicim.GuildID).members.cache.get(message.member.id).roles.add(setup.chatMute)
