...
channel.wsend({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(takachicim.GuildID).name, iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
...
channel.wsend({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(takachicim.GuildID).name, iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({dynamic:true})}).setThumbnail(oldState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
...
channel2.wsend({ embeds: [new EmbedBuilder().setAuthor({ name: client.guilds.cache.get(takachicim.GuildID).name, iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({dynamic:true})}).setThumbnail(newState.member.displayAvatarURL({ dynamic: true, size: 2048 })) 
.setDescription(`
...
if (!oldState.serverMute && newState.serverMute) return channel3.wsend({ embeds: [
  new EmbedBuilder()
  .setDescription(`**${newState.member ? newState.member.user.tag : newState.member.user.displayName}** adlı kullanıcıya **${entry.executor.tag}** tarafından Sağ-tık susturma işlemi yapıldı.`)
  .addFields(
    { name: "Cezalandırılan",  value: `[${newState.member ? newState.member.user.tag : newState.member.user.displayName}](https://discord.com/users/${newState.member.user.id})`, inline: true },
