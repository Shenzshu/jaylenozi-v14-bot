...
module.exports = async (message) => {
  ...
  if (message.author.bot) return;
    ...
    await client.checkLevel(message.author.id, takachicim.GuildID, "mesaj")
	let data = await Stat.findOne({ userID: message.author.id, guildID: takachicim.GuildID }) || { messageLevel: 0 };
	...
	await Stat.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $set: {["messageXP"]: 0}, $inc: {["messageLevel"]: 1 }}, { upsert: true });
	await ozi.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: +para } }, { upsert: true });
	message.channel.send({ content: `> **[** \`${message.author.displayName}\` **]** kullanıcısı level atladı TEBRİKLER!`, files:  [attachment] });
return }
...
await Stat.findOneAndUpdate({ guildID: takachicim.GuildID, userID: id }, { $inc: { messageXP: randomMessageXP } }, { upsert: true });
};
