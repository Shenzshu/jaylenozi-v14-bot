...
`);
channel.wsend({ content: `${member.toString()} [\` ${member.id} \`]`, embeds: [embed]});
kanal.wsend({ content: `${member.toString()} üyesi ${conf.tag} tagımızı alarak ailemize katıldı! Ailemiz ${guild.members.cache.filter(x => x.user.displayName.includes(conf.tag)).size} kişi oldu!` }).then((e) => setTimeout(() => { e.delete(); }, 5000));
}
