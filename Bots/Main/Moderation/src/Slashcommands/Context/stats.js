...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const member = await client.guilds.cache.get(takachicim.GuildID).members.fetch(interaction.targetId)
    ...
    const ctx = canvas.getContext('2d');
     ...
     let ozi = Math.floor((Math.random() * ZediN.length));
      ...
      if(member.user.displayName.length > 8) {
        ...
        yazı.push(`${member.user.displayName}`)
                }
