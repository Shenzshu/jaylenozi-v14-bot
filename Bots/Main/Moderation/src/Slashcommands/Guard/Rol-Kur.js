...
module.exports = {
    ...
    async execute(interaction, bot) {
      ...
      const RoleData = await RoleModel.findOne({ guildID: takachicim.GuildID, roleID: victim });
...
`, ephemeral: true })
               ...
               for (let index = 0; index < availableBots.length; index++) {
                  ...
                  if(!member){
                   ...
                   return true;}
                   await member.roles.add(yeniRol.id).then(e => {console.log(`[${victim}] Rol kurulumundan sonra ${bot.user.tag} - ${member.user.displayName} adlı üye ${yeniRol.name} rolünü aldı.`);}).catch(e => {console.log(`[${yeniRol.id}] Olayından sonra ${bot.user.username} - ${member.user.displayName} adlı üyeye rol veremedim.`);});});
