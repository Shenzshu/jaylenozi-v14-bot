...
const CategoryChannels = require("../../../../../Guard/src/Models/CategoryChannels");
const TextChannels = require("../../../../../Guard/src/Models/TextChannels");
const VoiceChannels = require("../../../../../Guard/src/Models/VoiceChannels");
const RoleModel = require("../../../../../Guard/src/Models/Role");
const takachicim = require("../../../../../../config.json");
const moment = require("moment");
require("moment-duration-format");
const client = global.bot;
const Bots = require("../../../../../Guard/Backup")
...
module.exports = {
  ...
  async execute(interaction, bot) {
   if(!takachicim.owners.includes(interaction.user.id)) {
       ...
       const guild = client.guilds.cache.get(takachicim.GuildID);
        ...
        if (ozi === "roles") {
          ...
          interaction.reply({ content: `Son 1 saat içinde **${denetim.length || 0}** adet rol silinmiş.`, components: [row], ephemeral: true })        
            ...
            if(kurulacakRole) {
            const RoleData = await RoleModel.findOne({ guildID: takachicim.GuildID, roleID: kurulacakRole });
...
İşlemin biteceği tahmini süre: ${(length>1000 ? parseInt((length*(takachicim.Guard.Guard.GiveRoleDelay/1000)) / 60)+" dakika" : parseInt(length*(takachicim.Guard.Guard.GiveRoleDelay/1000))+" saniye")}
\`\`\``, ephemeral: true })
                     ...
                     for (let index = 0; index < availableBots.length; index++) {
                         ...
                         let guild = bot.guilds.cache.get(takachicim.GuildID); 
                         ids.every(async id => {
                        let member = guild.members.cache.get(id);
                        if(!member){
                         console.log(`[${kurulacakRole}] Rol Kurulumundan sonra ${bot.user.username} - ${id} adlı üyeyi sunucuda bulamadım.`);
                         return true;}
                         await member.roles.add(yeniRol.id).then(e => {console.log(`[${kurulacakRole}] Rol kurulumundan sonra ${bot.user.tag} - ${member.user.displayName} adlı üye ${yeniRol.name} rolünü aldı.`);}).catch(e => {console.log(`[${yeniRol.id}] Olayından sonra ${bot.user.username} - ${member.user.displayName} adlı üyeye rol veremedim.`);});});
