...
await guild.members.fetch();
  ...
  const { joinVoiceChannel, getVoiceConnection} = require("@discordjs/voice");
    ...
    setInterval(() => client.user.setActivity({ name: `${activities[i++ % activities.length]}`,
      ...
      url: "https://www.twitch.tv/reazgann"}), 10000);
  ...
  finishedPenals.forEach(async (x) => {
    ...
    if (x.type === "CHAT-MUTE") { 
      ...
      client.channels.cache.get(conf.cmuteLogChannel).send({ embeds: [
        new EmbedBuilder()
        .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **Chat-Mute** cezası süresi bittiği için kaldırıldı.`)
        .addFields(
    ...
    if (x.type === "TEMP-JAIL") {
      ...
      client.channels.cache.get(conf.jailLogChannel).send({ embeds: [
        new EmbedBuilder()
        .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **Jail** cezası süresi bittiği için kaldırıldı.`)
        .addFields(
          { name: "Affedilen",  value: `[${member ? member.user.tag : member.user.displayName}](https://discord.com/users/${member.user.id})`, inline: true },
          { name: "Ceza Bitiş",  value: `<t:${Math.floor((Date.now()) / 1000)}:R>`, inline: true },
          )
    ...
    if (x.type === "VOICE-MUTE") {
      ...
      client.channels.cache.get(conf.vmuteLogChannel).send({ embeds: [
        new EmbedBuilder()
        .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **Voice-Mute** cezası süresi bittiği için kaldırıldı.`)
        .addFields(
          { name: "Affedilen",  value: `[${member ? member.user.tag : member.user.displayName}](https://discord.com/users/${member.user.id})`, inline: true },
          { name: "Ceza Bitiş",  value: `<t:${Math.floor((Date.now()) / 1000)}:R>`, inline: true },
          )
