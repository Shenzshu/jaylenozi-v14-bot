...
module.exports = async (message) => {
  let prefix = takachicim.Main.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  ...
  const embed = new EmbedBuilder().setFooter({ text: takachicim.AltBaşlık}).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    if (cmd.conf.owner && !takachicim.owners.includes(message.author.id)) return;
