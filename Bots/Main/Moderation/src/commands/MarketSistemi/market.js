...
let ZediN = await message.channel.send({ embeds: [ozi],  components: [market, market2] });
    var filter = (xd) => xd.user.id === message.author.id;
   
    let collector = await ZediN.createMessageComponentCollector({filter,  time: 30000 })
...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
        ...
        client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Spotify Premium\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: -40000 } }, { upsert: true });
         
      ZediN.edit({ embeds: [spotify], components: []}); 
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Netflix UHD\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: -50000 } }, { upsert: true });
         
      ZediN.edit({ embeds: [netflix], components: []}); 
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Youtube Premium\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: -60000 } }, { upsert: true });

      ZediN.edit({ embeds: [youtube], components: []}); 
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Classic Nitro\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: -125000 } }, { upsert: true });

      ZediN.edit({ embeds: [cnitro], components: []}); 
...
client.channels.cache.find(x => x.name == "market_log").send(`${message.author} kişisi \`Boostlu Nitro\` ürününü satın aldı. İletişime geçmenizi bekliyor! :tada:`)
         await dolar.findOneAndUpdate({ guildID: takachicim.GuildID, userID: message.author.id }, { $inc: { dolar: -150000 } }, { upsert: true });

      ZediN.edit({ embeds: [bnitro], components: []}); 

        }

      if (button.customId == "çıkış") {
      await ZediN.delete({ timeout: 1500 });
      }
