...
run: async (client, message, args, embed, prefix) => {
    ...
    if (gunluk !== null && timeout - (Date.now() - gunluk) > 0) {
        ...
        message.reply({ content:`:stopwatch: **|** Hata! **${message.author.displayName}** Bu komutu ${time} sonra kullanabilirsin.`})
	} else {
    ...
    if(dolarData.dolar && 50000 < sec) return message.reply({ content:`:no_entry: | **${message.author.displayName}**, Yeterli miktar da paran yoktur! (Max: 50.000 Tutarında Oynayabilirsin)`}) 
