...
module.exports = {
    ...
    return }
       ...
       const etkinlik = await client.guilds.cache.get(takachicim.GuildID).roles.cache.find(x => x.name.includes(conf.etkinlik)).id
       const cekilis = await client.guilds.cache.get(takachicim.GuildID).roles.cache.find(x => x.name.includes(conf.cekilis)).id
