...
Toplam tag ve etiket tagında bulunan **${message.guild.members.cache.filter(m => m.user.discriminator.includes(sayitagi)).size + message.guild.members.cache.filter(m => m.user.displayName.toLowerCase().includes(tag)).size}** kişi tespit edildi. Kişilere <@&${role.id}> Rolü dağıtılıyor!
`)]
        })
        message.guild.members.cache.forEach(qwe => {
          if (qwe.user.displayName.includes(tag)) {
...
**Ekip Ses Aktifliği:** ${message.guild.members.cache.filter(s => s.user.discriminator.includes(value.Sayısı)).filter(s => s.voice.channel).size + message.guild.members.cache.filter(s => s.user.displayName.includes(value.EkipAdı)).filter(s => s.voice.channel).size || 0}
**─────────────────────────────────**
...
**İsminde Tag (${a.EkipAdı}) Olup Seste Olan Kişi Sayısı:** ${message.guild.members.cache.filter(s => s.user.displayName.toLowerCase().includes(a.EkipAdı)).filter(s => s.voice.channel).size || "0"}
...
**Toplam Ses Aktifliği: (${a.EkipAdı} - #${a.Sayısı})** ${message.guild.members.cache.filter(s => s.user.discriminator.includes(a.Sayısı)).filter(s => s.voice.channel).size + message.guild.members.cache.filter(s => s.user.displayName.includes(a.EkipAdı)).filter(s => s.voice.channel).size || 0}`)
      if (!ekipler) {
