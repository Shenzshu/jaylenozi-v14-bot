...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        let ZediN = table(xd.slice(0, 15), config)
    ...
    if (button.customId === "dosya") {
      ...
      button.reply({ content: `${member} kullanıcısının toplamda **${cezaData ? cezaData.ceza.length : 0}** cezası  bulunmaktadır ve aşağıdaki belgede yazmaktadır.`, files: [{ attachment: Buffer.from(ozi), name: `${member.user.displayName}_sicil.txt` }], components: [], ephemeral: true }); 
