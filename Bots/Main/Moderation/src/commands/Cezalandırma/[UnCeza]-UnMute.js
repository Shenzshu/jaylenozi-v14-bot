...
module.exports = {
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "mute") {
      ...
      if (takachicim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
      ...
      .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **${message.author.tag}** tarafından Chat Mute cezası kaldırıldı.`)
      .addFields(
        { name: "Affedilen",  value: `[${member ? member.user.tag : member.user.displayName}](https://discord.com/users/${member.user.id})`, inline: true },
    ...
    if (button.customId === "vmute") {
      ...
      if (takachicim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından **sesli kanallarda** olan susturmanız kaldırıldı!`}).catch(() => {});
      ...
      const log = new EmbedBuilder()
      .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **${message.author.tag}** tarafından Ses Mute cezası kaldırıldı.`)
      .addFields(
        { name: "Affedilen",  value: `[${member ? member.user.tag : member.user.displayName}](https://discord.com/users/${member.user.id})`, inline: true },
        { name: "Affeden",  value: `[${message.author.tag}](https://discord.com/users/${message.author.id})`, inline: true },
        { name: "Ceza Bitiş",  value: `<t:${Math.floor((Date.now()) / 1000)}:R>`, inline: true },
