...
module.exports = {
  ...
  run: async (client, message,  args, embed) => {
    ...
    if (takachicim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, jailiniz kaldırıldı!`}).catch(() => {});

          const log = new EmbedBuilder()
          .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcının **${message.author.tag}** tarafından Jail cezası kaldırıldı.`)
          .addFields(
