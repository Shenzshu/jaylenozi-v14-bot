...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (takachicim.Main.dmMessages) user.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
    ...
    message.reply({ content :`${green} ${member ? member.toString() : user.displayName} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    message.react(green)
