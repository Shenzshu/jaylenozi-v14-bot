...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (takachicim.Main.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == takachicim.Main.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!"}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    message.react(green)
    if (takachicim.Main.dmMessages) user.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle banlandınız!`}).catch(() => {});
    ...
    .setDescription(`**${member ? member.toString() : user.displayName}** Üyesi Sunucudan **${reason}** Sebebiyle \n${message.author} Tarafından banlandı! Doğrular tektir değişmez ! Ceza Numarası: (\`#${penal.id}\`)`)

    message.react(green)
    message.reply({ embeds: [xd]});
 
    const log = new EmbedBuilder()
      .setDescription(`**${member ? member.user.tag : user.displayName}** adlı kullanıcı **${message.author.tag}** tarafından banlandı.`)
      .addFields(
        { name: "Cezalandırılan",  value: `[${member ? member.user.tag : user.displayName}](https://discord.com/users/${user.id})`, inline: true },
