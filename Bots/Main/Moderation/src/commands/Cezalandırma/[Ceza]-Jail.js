...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (takachicim.Main.jaillimit > 0 && jailLimit.has(message.author.id) && jailLimit.get(message.author.id) == takachicim.Main.jaillimit) 
    {
    message.react(red)
...
if (duration) {
  ...
  if (takachicim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar jaillendiniz.`}).catch(() => {});
  
  const log = new EmbedBuilder()
  .setDescription(`**${member ? member.user.tag : member.user.displayName}** adlı kullanıcı **${message.author.tag}** tarafından Jail atıldı.`)
  .addFields(
  ...
  if(!message.member.permissions.has(PermissionsBitField.Flags.AddReactions) && !conf.sahipRolu.some(x => message.member.roles.cache.has(x))) {
    if (takachicim.Main.jaillimit > 0) {
