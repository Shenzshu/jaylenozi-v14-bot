...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
   ...
   for(const sex of [...yarrak.values()]){
     ...
     .setDescription(`**${sex ? sex.user.tag : sex.user.displayName}** adlı kullanıcının ban cezası kaldırıldı.`)
     .addFields(
       { name: "Affedilen",  value: `[${sex ? sex.user.tag : sex.user.displayName}](https://discord.com/users/${sex.user.id})`, inline: true },
   ...
   await message.guild.members.unban(sex.user.id, `${message.author.displayName} tarafından kaldırıldı!`).catch(() => {});
