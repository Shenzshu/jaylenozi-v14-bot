...
if (takachicim.Main.dmMessages) member.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
...
msg.edit({ content :`${green} ${member ? member.toString() : member.user.displayName} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

const log = new EmbedBuilder() 
.setDescription(`
${Cezaa} Banlanan Üye: **${member ? member.toString() : member.user.displayName}** \`${member.user.id}\`
...
if(button.customId === "üç") {
  ...
  return }
  if (takachicim.Main.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == takachicim.Main.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!", ephemeral: true })

  if (takachicim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **Reklam** sebebiyle banlandınız!`}).catch(() => {});
  ...
  msg.edit({ content :`${green} ${member ? member.toString() : member.user.displayName} üyesi, ${message.author} tarafından, \`Reklam\` nedeniyle banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})
