...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    return }
    message.guild.members.unban(args[0], `${message.author.displayName} tarafından kaldırıldı!`).catch(() => {});
    ...
    message.react(green)
    message.reply({ content:`${green} \`(${ban.user.displayName.replace(/\`/g, "")} - ${ban.user.id})\` adlı üyenin banı ${message.author} tarafından kaldırıldı!`}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (takachicim.Main.dmMessages) ban.user.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından banınız kaldırıldı!`}).catch(() => {});

    const log = new EmbedBuilder()
    .setDescription(`**${ban ? ban.user.tag : ban.user.displayName}** adlı kullanıcının **${message.author.tag}** tarafından ban cezası kaldırıldı.`)
    .addFields(
      { name: "Affedilen",  value: `[${ban ? ban.user.tag : ban.user.displayName}](https://discord.com/users/${ban.user.id})`, inline: true },
