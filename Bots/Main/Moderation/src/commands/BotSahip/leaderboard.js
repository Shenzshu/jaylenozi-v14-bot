...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const messageUsersData = await messageUser.find({ guildID: takachicim.GuildID }).sort({ topStat: -1 });
    const messageUsers = messageUsersData.splice(0, 20).map((x, index) => `\` ${index+1} \` <@${x.userID}>: \`${Number(x.topStat).toLocaleString()} mesaj\``).join(`\n`);
    const voiceUsersData = await voiceUser.find({ guildID: takachicim.GuildID }).sort({ topStat: -1 });
    ...
    const sunucuisim = client.guilds.cache.get(takachicim.GuildID).name
   ...
   .setAuthor({ name: client.guilds.cache.get(takachicim.GuildID).name, iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({dynamic:true})})
   ...
   LeaderBoard.send({ embeds: [Voice]}).then(async (vmsg) => {
    ...
    children.exec(`pm2 restart ${takachicim.GuildName}_Statistics`);
  })
