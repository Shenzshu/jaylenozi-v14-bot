...
client.on('interactionCreate', async interaction => {
    if(interaction.customId === "create") {
        ...
        const xd = await client.guilds.cache.get(takachicim.GuildID).channels.cache.get(data?.ID)
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    return }
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(takachicim.GuildID, { Connect: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(takachicim.GuildID, { Connect: true });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(takachicim.GuildID, { ViewChannel: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(takachicim.GuildID, { ViewChannel: true });
        ...
        if (button.customId === "onays") {
            ...
            .setAuthor({ name: client.guilds.cache.get(takachicim.GuildID).name, iconURL: client.guilds.cache.get(takachicim.GuildID).iconURL({ dynamic: true, size: 2048 })})
            .setFooter({ text: interaction.user.displayName, iconURL: interaction.user.avatarURL({ dynamic: true }) })
            .setTimestamp()
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                x.values.map(async (v) => {
                    ...
                    let memb = client.guilds.cache.get(takachicim.GuildID).members.cache.get(v)
...
client.on("ready", async () => {
    const sunucu = client.guilds.cache.get(takachicim.GuildID);
