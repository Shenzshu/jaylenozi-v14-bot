...
client
  .login(takachicim.Main.ModerationToken)
  ...
  fs.readdirSync('./src/Slashcommands/').forEach(async category => {
	...
	const rest = new REST({ version: '10' }).setToken(takachicim.Main.ModerationToken);
  (async () => {
	try {
		console.log('[OZI] Slash ve Komutlar yükleniyor.');
		await rest.put(
			Routes.applicationGuildCommands(takachicim.Main.BotClientID, takachicim.GuildID),
			{ body: slashcommands },
		).then(() => {
			console.log('[OZI] Slash ve Context Komutlar yüklendi.');
		});
	}
	catch (e) {
		console.error(e);
...
const xd = [
    takachicim.Main.ModerationToken,
    takachicim.Main.RegisterToken,
    takachicim.Main.StatsToken,
    takachicim.Guard.Token.Guard_I,
    takachicim.Guard.Token.Guard_II,
    takachicim.Guard.Token.Guard_III
];
xd.forEach(xxx => 
tkn.push(xxx)
)
